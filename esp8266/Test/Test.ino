...
float alfa;
float beta;
float calibrated_Alfa = 3.95;
float calibrated_Beta = -2.65;
const uint8_t scl = D1;                   // Select SDA and SCL pins for I2C communication
...
uint32_t m_conversionDelay = 8;                             // 5ms for conversion time
int16_t adc2,adc3;
...
void loop()
{
    if (prvTime - millis() > 100) // 20 ms time sample
    {
        prvTime = millis();
        // Read Sensors
        ...
        ComplementaryFilter(accelData, gyroData, &alfa, &beta);
        //alfa = alfa - calibrated_Alfa;
        // beta = beta - calibrated_Beta;
        ...
        Serial.print((int)adc2);
        Serial.print(" \tCurrent M2: ");
        Serial.println((int)adc3);
    }
}
...
// ADS1015 configuration and communication for reading a channel 
